...
function buildSystemPrompt() {
  return [
    ...
    '{"axis": "<etiqueta exacta de la lista>", "reason": "<una frase: por que ese tema, citando el articulo si lo hay>"}',
    "",
    ...
    `- Si el aporte es "${OUT_OF_SCOPE_TOPIC.label}" y ademas se relaciona con otro tema, decilo en reason asi: "El Codigo no lo regula, pero se relaciona con <tema> (<articulo>)".`,
...
export async function classifyContributionTopic(input: {
  ...
  try {
    ...
    const parsed = JSON.parse(response.answer) as { axis?: unknown; reason?: unknown };
