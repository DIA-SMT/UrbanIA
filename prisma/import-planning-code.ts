...
async function main() {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    const source = await tx.knowledgeSource.upsert({ where: { kind_externalId: { kind: KnowledgeSourceKind.REGULATION, externalId: SOURCE_ID } }, update: { title: "Código de Planeamiento Urbano vigente (Ord. 2648/98, texto ordenado 2014 – último disponible)", filePath: parsed.document.sourcePath, mimeType: "text/plain", status: ProcessingStatus.READY, rawText, wordCount: rawText.split(/\s+/).length, processedAt: new Date(), metadata: { documentId: parsed.document.id, ordinanceNumber: parsed.document.ordinanceNumber, version: "2014-05", structured: true } }, create: { kind: KnowledgeSourceKind.REGULATION, externalId: SOURCE_ID, title: "Código de Planeamiento Urbano vigente (Ord. 2648/98, texto ordenado 2014 – último disponible)", filePath: parsed.document.sourcePath, mimeType: "text/plain", status: ProcessingStatus.READY, rawText, wordCount: rawText.split(/\s+/).length, processedAt: new Date(), metadata: { documentId: parsed.document.id, ordinanceNumber: parsed.document.ordinanceNumber, version: "2014-05", structured: true } } });
