    ...
    <AppShell>
      <section className="urban-card overflow-hidden rounded-lg">
        <div className="grid gap-6 p-5 lg:grid-cols-[minmax(0,1fr)_340px] lg:p-7">
          <div>
            <div className="mb-4 inline-flex items-center gap-2 rounded-md border border-emerald-300/20 bg-emerald-300/10 px-3 py-2 text-xs font-bold uppercase tracking-[0.14em] text-emerald-200">
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              <button className="inline-flex items-center gap-2 rounded-md bg-emerald-500 px-4 py-3 text-sm font-bold text-civic-ink">
                <Plus className="h-4 w-4" />
                {page.primaryAction}
              </button>
              <button className="inline-flex items-center gap-2 rounded-md border border-white/10 bg-white/[0.04] px-4 py-3 text-sm font-semibold text-slate-200">
                Ver tablero
          ...
          <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-1">
            {page.stats.map((stat) => (
              <div key={stat.label} className="rounded-md border border-white/10 bg-slate-950/50 p-4">
      ...
      <section className="mt-4 grid gap-4 lg:grid-cols-[minmax(0,1fr)_320px]">
        <div className="grid gap-4 md:grid-cols-3">
          {page.cards.map((card) => (
            <article key={card.title} className="urban-card min-w-0 rounded-lg p-5">
        ...
        <aside className="urban-card rounded-lg p-5">
          ...
          <div className="mt-5 space-y-3">
            {["Estructura visual lista", "Modelo de datos inicial", "Integracion futura con IA"].map((item) => (
              <div key={item} className="flex items-center gap-3 rounded-md border border-white/8 bg-white/[0.03] px-3 py-3 text-sm text-slate-300">
