...
export type MapPickCoords = { lat: number; lng: number };
export type UrbanLeafletMapProps = {
  mode?: "explore" | "pick";
  value?: MapPickCoords | null;
  onPick?: (coords: MapPickCoords) => void;
};

// Captura clicks para ubicar un punto (modo picker) y dibuja el marcador elegido.
function PickLayer({ value, onPick }: { value?: MapPickCoords | null; onPick?: (coords: MapPickCoords) => void }) {
  useMapEvents({ click: (event) => onPick?.({ lat: event.latlng.lat, lng: event.latlng.lng }) });
  if (!value) return null;
  return <CircleMarker center={[value.lat, value.lng]} radius={9} pathOptions={{ color: "#ffffff", weight: 3, fillColor: "#1f89f6", fillOpacity: 1 }} />;
}

export function UrbanLeafletMap({ mode = "explore", value = null, onPick }: UrbanLeafletMapProps = {}) {
  ...
  return <section className={`relative isolate overflow-hidden rounded-2xl border border-slate-200 bg-slate-100 shadow-sm dark:border-white/10 dark:bg-[#071724] ${isPick ? "h-[380px]" : "h-[calc(100vh-190px)] min-h-[620px]"}`}>
    <MapContainer center={initialCenter} zoom={isPick ? 14 : 13} minZoom={10} scrollWheelZoom preferCanvas className="h-full w-full" zoomControl={false}>
      <TileLayer attribution='&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a>' url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" />
      {isPick ? (
        <PickLayer value={value} onPick={onPick} />
      ) : (
        <>
