    ...
    <section className="urban-card urban-lift relative min-h-[430px] overflow-hidden rounded-lg">
      ...
      <div className="relative z-10 grid min-h-[430px] grid-cols-1 lg:grid-cols-[340px_1fr]">
        <div className="m-4 rounded-lg md:m-5 border border-white/10 bg-slate-950/62 p-5 backdrop-blur">
          ...
          <div className="urban-lift mt-5 flex items-center gap-3 rounded-md border border-white/10 bg-slate-950/70 px-3 py-3 text-sm text-slate-400">
        ...
        <div className="relative min-h-[260px]">
          <div className="absolute right-5 top-5 flex gap-2">
            {["2D", "3D"].map((item) => (
              <button key={item} className={`urban-button rounded-md px-3 py-2 text-sm font-semibold ${item === "3D" ? "bg-cyan-400/20 text-cyan-100" : "bg-slate-950/70 text-slate-300"}`}>{item}</button>
            ))}
            <button className="urban-button rounded-md bg-slate-950/70 p-2"><Layers3 className="h-5 w-5" /></button>
          </div>
          {pins.map((pin) => (
            <div key={pin.label} className={`urban-pin absolute grid h-12 w-12 md:h-14 md:w-14 place-items-center rounded-full border-4 border-white/75 text-white shadow-glow ${pin.className}`}>
          ...
          <div className="absolute bottom-5 left-1/2 flex w-[calc(100%-2rem)] max-w-xl -translate-x-1/2 flex-wrap justify-center gap-2 rounded-md border border-white/10 bg-slate-950/72 p-2 backdrop-blur">
            {["Transporte", "Espacios verdes", "Equipamiento", "Zonificacion", "Riesgos"].map((layer) => (
              <span key={layer} className="urban-lift rounded-md bg-white/7 px-3 py-2 text-xs font-semibold text-slate-200">{layer}</span>
            ))}
          </div>
        </div>
      </div>
