...
export default async function SolicitudesPage() {
  ...
  const sidituc = siditucIntegrationStatus();
    ...
    <div>
      <div className="mb-4">
        ...
        <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
          Pedidos de ingreso al sistema interno de personas ya identificadas en SIDITUC, a la espera de que un administrador les asigne rol.
        </p>
      </div>
      ...
      {!sidituc.enabled ? (
        <div className="mb-4 flex items-start gap-3 rounded-2xl border border-sky-200 bg-sky-50 p-4 dark:border-sky-400/30 dark:bg-sky-400/10">
          ...
          <div>
            <p className="text-sm font-bold text-sky-900 dark:text-sky-100">La bandeja se activa con la integración SIDITUC</p>
            <p className="mt-1 text-xs leading-5 text-sky-800/80 dark:text-sky-200/80">
              La estructura ya está lista: cuando la integración esté operativa, cada persona validada en SIDITUC que pida acceso aparecerá acá para aprobarse o rechazarse.{" "}
              <Link href="/admin/configuracion/sidituc" className="font-bold underline decoration-sky-400 underline-offset-2">Ver estado de la integración</Link>.
            </p>
          </div>
        </div>
      ) : null}
        ...
        {requests.length === 0 ? (
          <div className="grid place-items-center px-6 py-16 text-center">
            ...
            <p className="mt-1 max-w-md text-xs leading-5 text-slate-400 dark:text-slate-500">
              Cuando alguien solicite acceso al sistema interno, va a aparecer en esta bandeja con su identidad SIDITUC verificada.
            </p>
          </div>
        ) : (
            ...
            <table className="w-full min-w-[760px] text-left text-sm">
              <thead>
                <tr className="border-b border-slate-200/80 text-[11px] font-black uppercase tracking-[0.08em] text-slate-400 dark:border-white/10 dark:text-slate-500">
                  ...
                  <th scope="col" className="px-3 py-3">Estado SIDITUC</th>
              ...
              <tbody>
                {requests.map((request) => (
                  <tr key={request.id} className="border-b border-slate-100 last:border-0 dark:border-white/5">
                    ...
                    <td className="px-3 py-3 text-slate-600 dark:text-slate-300">{request.siditucStatus ?? "—"}</td>
